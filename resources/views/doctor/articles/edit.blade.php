...
<head>
    ...
    <title>Edit Artikel Dokter - KuloSehat</title>
                ...
                <div class="form-card px-6 py-10 sm:px-10 md:px-14">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
                        <div>
                            ...
                            <h1 class="mt-2 text-2xl sm:text-3xl font-bold text-gray-900">Edit Artikel</h1>
                            <p class="text-sm text-gray-500 mt-1">Perbarui konten edukasi yang sudah Anda publikasikan.</p>
                        </div>
                    ...
                    <form action="{{ route('doctor.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            @if ($article->image)
                                <img src="{{ asset('storage/' . $article->image) }}" alt="{{ $article->title }}" class="w-48 h-auto rounded-2xl my-3 object-cover">
                            @else
                        ...
                        <div class="flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="rounded-full bg-green-500 px-6 py-2 text-sm font-semibold text-white shadow-lg shadow-green-500/30 hover:bg-green-600">Perbarui Artikel</button>
                        </div>
                    </form>
                </div>
            </div>
